...
export function AnonShortenForm() {
  ...
  const handleCopy = () => {
    navigator.clipboard.writeText(createLink.data?.data.short_url ?? "");
    setCopied(true);
  ...
  return (
    <div className="w-full max-w-2xl mx-auto space-y-3">
      <div className="rounded-2xl p-4 sm:p-5 bg-[#0f1629] border border-white/6">
        ...
        {createLink.data && (
          <div className="rounded-2xl mt-3 p-4 sm:p-5 flex flex-col sm:flex-row items-start sm:items-center gap-3 bg-[#0f1629] border border-indigo-500/30">
            <div className="flex items-center gap-2.5 flex-1 min-w-0">
              <span className="w-2 h-2 rounded-full bg-emerald-400 shrink-0 animate-pulse" />
              <code className="text-base font-bold font-mono text-indigo-400 break-all">
                {createLink.data.data.short_url}
              </code>
            </div>
            <button
              onClick={handleCopy}
